...
#Function defined to calculate variable variables depending on Theta2:
    def calculate(self,theta2):
        ...
        arg = (L3**2 + L4**2 - BD**2) / (2 * L3 * L4)

        # Clamp the value to the range [-1, 1]
        arg_clamped = np.clip(arg, -1, 1)

        # Calculate alfa using the clamped value
        alfa = np.arccos(arg_clamped)
        ...
        numerator3 = (-L2 * np.sin(theta2)) + L4 * np.sin(alfa)
        denominator3 = L1 + L3 - L2 * np.cos(theta2) - L4 * np.cos(alfa)

        # Add epsilon to the denominator to avoid division by zero
        denominator_safe3 = denominator3 + epsilon * np.sign(denominator3)

        # Calculate theta4 using the safe denominator
        theta3 = 2 * np.arctan(numerator3 / denominator_safe3)
        ...
        # Calculate the numerator and denominator
        numerator4 = L2 * np.sin(theta2) - L3 * np.sin(alfa)
        denominator4 = L4 - L1 + L2 * np.cos(theta2) - L3 * np.cos(alfa)

        # Add epsilon to the denominator to avoid division by zero
        denominator_safe4 = denominator4 + epsilon * np.sign(denominator4)

        # Calculate theta4 using the safe denominator
        theta4 = 2 * np.arctan(numerator4 / denominator_safe4)
...
#--------------------------------------------------------------
#Function to check grashof and linkage type
    def grashof(self):
        L1 = self.L1
        L2 = self.L2
        L3 = self.L3
...
#When the start button is pressed:
    def start(self): #Since the variable k is stored in self, it is sufficient to take self as input instead of self.k!
        ...
        try:
            ...
            self.warning.config(text=self.grashof(),background="white",font=("Courier", 14))
        except:
